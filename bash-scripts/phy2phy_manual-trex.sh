...
#    sudo insmod $DPDK_IGB_UIO
    echo "Binding the ports to vfio-pci"
    sudo $DPDK_BIND_TOOL --bind=vfio-pci $DPDK_PCI1 $DPDK_PCI2
    ...
    sudo $OVS_DIR/utilities/ovs-vsctl --no-wait set Open_vSwitch . other_config:dpdk-socket-mem="4096,0"
    ...
    sudo $OVS_DIR/utilities/ovs-vsctl --no-wait set Open_vSwitch . other_config:dpdk-isolate-mem=true
    sudo $OVS_DIR/utilities/ovs-vsctl --no-wait set Open_vSwitch . other_config:dpdk-whitelist-pci-ids="${DPDK_PCI1},${DPDK_PCI2}"
